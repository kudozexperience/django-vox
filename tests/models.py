...
class User(VoxModel, AbstractBaseUser, PermissionsMixin):
    ...
    def __activity__(self):
        return aspy.Person(id=self.get_object_address(), name=self.name)
    ...
    def make_vcard(self) -> str:
        ...
        for field in ("name", "email"):
            params[field] = getattr(self, field).replace(":", "\\:").replace(";", "\\;")
...
class Subscriber(VoxModel):
    ...
    class VoxMeta:
        notifications = VoxNotifications(
            create=VoxNotification(
                _(
                    "Notification from subscriber that a new subscriber "
...
objects[Article].channels.add("author", _("Author"), User, Article.get_authors)
